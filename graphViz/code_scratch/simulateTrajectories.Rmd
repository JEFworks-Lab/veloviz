...
set.seed(1)
out <- generate_dataset(model = "multifurcating", num_cells = 1000, num_features = 1500) 
exp = out$expression
...
```{r}
pca = svds(A = exp, k=50, opts = list(center = TRUE, scale = TRUE, maxitr = 2000, tol = 1e-10))
var = pca$d
plot(var) # ~3 components 
pcs = pca$u
rownames(pcs) = rownames(exp)
emb.pca = pcs[,1:2]
plot(emb.pca,pch=4)

com = getComMembership(pcs[,1:3], k=100, method = igraph::cluster_louvain)
...
plot(scale(emb.pca), pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols, main="PCA: simulated trajectories")
legend(x = -0.5, y= -0.5, legend = levels(cluster.label), col = cluster.cols, pch = 16)

set.seed(1)
umap = umap(pca$u[,1:10])$layout
plot(umap,pch=4,col = cell.cols)

```

Assign pseudotime to cells  
```{r assign pseudotime, fig.width=8, fig.height=5}
#assign pseudotime
emb.pca.scale = scale(emb.pca)
#divide b/w y axis
emb.pca.scale.neg = emb.pca.scale[emb.pca.scale[,1]<0.1,]
emb.pca.scale.pos = emb.pca.scale[emb.pca.scale[,1]>0.1,]
#root cell for both halves 
cell0neg = rownames(emb.pca.scale.neg)[which(emb.pca.scale.neg[,2]==min(emb.pca.scale.neg[,2]))]
cell0pos = rownames(emb.pca.scale.pos)[which(emb.pca.scale.pos[,2]==max(emb.pca.scale.pos[,2]))]
#distance from root cell for both halves 
diffneg = emb.pca.scale.neg - matrix(emb.pca.scale.neg[cell0neg,], nrow = nrow(emb.pca.scale.neg), ncol = 2, byrow = TRUE)
diffpos = emb.pca.scale.pos - matrix(emb.pca.scale.pos[cell0pos,], nrow = nrow(emb.pca.scale.pos), ncol = 2, byrow = TRUE)
ptimeNeg = sqrt(diffneg[,1]^2 + diffneg[,2]^2)
ptimePos = sqrt(diffpos[,1]^2 + diffpos[,2]^2)
ptimePos = ptimePos + max(ptimeNeg)
ptimeall = c(ptimeNeg, ptimePos)
ptimeall = ptimeall[rownames(emb.pca.scale)]
#plot 
col.gradient = colorRampPalette(c('blue','red'))
ptime.col = col.gradient(1000)[as.numeric(cut(ptimeall,breaks = 1000))]
par(mfrow= c(1,2))
plot(emb.pca.scale, pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols, main="PCA: simulated trajectories")
plot(emb.pca.scale, pch=16, xlab = 'PC1', ylab = 'PC2', col = ptime.col, main="PCA: simulated trajectories")
points(x = emb.pca.scale[cell0neg,1], y = emb.pca.scale[cell0neg,2],pch = 16, col = "cyan",cex = 2)
points(x = emb.pca.scale[cell0pos,1], y = emb.pca.scale[cell0pos,2],pch = 16, col = "cyan",cex = 2)


#previous try with just dist from bottom left 
# cell0 = rownames(pcs)[which(emb.pca.scale[,2]==min(emb.pca.scale[,2]))]
# cell0.rep = matrix(emb.pca.scale[cell0,],nrow = 1000,ncol =2, byrow = TRUE)
# diff = emb.pca.scale - cell0.rep
# ptime = sqrt(diff[,1]^2 + diff[,2]^2)
# col.gradient = colorRampPalette(c('blue','red'))
# ptime.col = col.gradient(1000)[as.numeric(cut(ptime,breaks = 1000))]
# par(mfrow= c(1,2))
# plot(scale(emb.pca), pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols, main="PCA: simulated trajectories")
# plot(emb.pca.scale, pch=16, xlab = 'PC1', ylab = 'PC2', col = ptime.col, main="PCA: simulated trajectories")
# points(x = emb.pca.scale[cell0neg,1], y = emb.pca.scale[cell0neg,2],pch = 16, col = "cyan",cex = 2)
...
```{r}
#order cells wrt pseudotime 
ptime = sort(ptimeall)
cells.ordered = names(ptime)

#subset cells --> observed, projected cells
# cells.observed = cells.ordered[seq(1,1000,by=5)]
# cells.observed = cells.observed[1:length(cells.observed)-1]
# cells.projected = cells.ordered[seq(1,1000,by=5)-1]

tgap = 5
cells.observed = cells.ordered[1:(length(cells.ordered)-tgap)]
cells.projected = cells.ordered[(tgap+1):length(cells.ordered)]

plot(emb.pca.scale[cells.observed,],pch = 1, xlab = 'PC1', ylab = 'PC2', col = "blue", main="PCA: simulated trajectories")
points(emb.pca.scale[cells.projected,], pch = 4, col = "red")
points(emb.pca.scale[cells.observed[c(20,116,195,315,474,582,690,720,800,935)],], pch = 18, col = "black",cex = 1.5)
points(emb.pca.scale[cells.projected[c(20,116,195,315,474,582,690,720,800,935)],], pch = 18, col = "green", cex = 1.5)
legend(x = -1.8, y = 1.5, legend = c("observed","projected","observed","projected"),col = c("blue","red","black","green"),pch = c(1,4,18,18))


#subset expression --> observed, projected expression 
exp.curr = exp[cells.observed,]
exp.proj = exp[cells.projected,]
rownames(exp.proj) = rownames(exp.curr)

cell.cols.curr = cell.cols[cells.observed]
#cell.cols.proj = cell.cols[cells.projected]
```



```{r}
pca.curr = svds(A = exp.curr, k=50, opts = list(center = TRUE, scale = TRUE, maxitr = 2000, tol = 1e-10))
var = pca.curr$d
plot(var) # ~3 components 
pcs.curr = pca.curr$u
rownames(pcs.curr) = rownames(exp.curr)
emb.pca.cur = pcs.curr[,1:2]
#plot(emb.pca,pch=4)
plot(scale(emb.pca.cur), pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols.curr, main="PCA: simulated observed cells")

set.seed(1)
umap.curr = umap(pcs.curr[,1:10])$layout
plot(umap.curr,pch=16,col = cell.cols.curr)
```

```{r}
#observed and projected pca scores
v.curr = pca.curr$v 

curr.scores = exp.curr %*% v.curr
proj.scores = exp.proj %*% v.curr

par(mfrow = c(1,2))
plot(scale(curr.scores[,1:2]),pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols.curr, main="PCA: simulated observed cells") #xlim = c(-2,2),ylim = c(-2,3)
plot(scale(proj.scores[,1:2]),pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols.curr, main="PCA: simulated projected cells")
...
par(mfrow = c(1,2))
set.seed(1)
g = graphViz(observed = t(curr.scores), projected = t(proj.scores),
         k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = FALSE, 
         cell.colors = cell.cols.curr, title = "graph using PCA coordinates", 
...
```{r}
k = 10
pcs.fdg <- pcs.curr[,1:5]
nn = RANN::nn2(pcs.fdg, k = k) ## KNN
...
plot(scale(fdg),col = cell.cols.curr,pch = 16,main = "seed(1)")
set.seed(5)
fdg = layout_with_fr(g,dim=2)
colnames(fdg) = c("C1","C2")
plot(scale(fdg),col = cell.cols.curr,pch = 16, main = "seed(5)")
...
```{r}
#cells not in cluster 2 
not.clusterTwo.names = names(cluster.label)[which(cluster.label != 2)]

#observed cells not in cluster 2 
curr.not.clusterTwo.names = cells.observed[cells.observed %in% not.clusterTwo.names]
#observed and projected expression of cells not in 2
curr.not.clusterTwo = exp.curr[curr.not.clusterTwo.names,]
proj.not.clusterTwo = exp.proj[curr.not.clusterTwo.names,]

cols.notClusterTwo = cell.cols.curr[names(cell.cols.curr) %in% not.clusterTwo.names]

## run pca without current cell type 
pca.sub = svds(A = curr.not.clusterTwo, k=50, opts = list(center = TRUE, scale = FALSE, maxitr = 2000, tol = 1e-10))
var.sub = pca.sub$d
plot(var.sub) # ~3 components 

emb.pca.sub = pca.sub$u[,1:2]
par(mfrow = c(1,2))
plot(emb.pca.sub, col = cols.notClusterTwo, pch = 16, xlab = "PC1", ylab = "PC2", main = "PCA on observed w/o 2") # xlim = c(-0.04,0.07)
#legend(x=0.04, y=0.08, legend = levels(clusters), col = col, pch = 16)

set.seed(1)
umap.sub = umap(pca.sub$u[,1:10])$layout
plot(umap.sub,pch=16,col = cols.notClusterTwo)
...
curr.scores.types = curr.not.clusterTwo %*% v.currTypes
plot(scale(curr.scores.types[,1:2]), col = cols.notClusterTwo, main = 'Observed') #,xlim = c(-0.55,0.25),ylim = c(-0.35,0.45))

#proj.pca = proj[common.genes,pca.names.sub]
proj.scores.types = proj.not.clusterTwo %*% v.currTypes
plot(scale(proj.scores.types[,1:2]), col = cols.notClusterTwo, main = 'Projected') #,xlim = c(-0.55,0.25),ylim = c(-0.35,0.45))

source("projectedNeighbors.R")
par(mfrow = c(1,2))
set.seed(1)
graphViz(observed = t(curr.scores.types), projected = t(proj.scores.types),
         k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = FALSE,
         cell.colors = cols.notClusterTwo, title = "K = 10",
...
g <- igraph::graph_from_data_frame(nn.df, directed = TRUE)
g <- igraph::simplify(g)
set.seed(1)
fdg = layout_with_fr(g,dim=2)
par(mfrow = c(1,2))
plot.igraph(g,layout = fdg,vertex.label = NA , vertex.color = cols.notClusterTwo)
colnames(fdg) = c("C1","C2")
plot(scale(fdg),col = cols.notClusterTwo,pch = 16)
# set.seed(5)
# fdg = layout_with_fr(g,dim=2)
# colnames(fdg) = c("C1","C2")
# plot(scale(fdg),col = cols.notClusterTwo,pch = 16)
...
```{r}
mf = readRDS('./saved_objects/simulated_trajectories/multifurcating_4.rds')
emf = mf$expression
pca.emf = svds(A=emf,k=50, opts = list(center = TRUE, scale = TRUE, maxitr = 2000, tol = 1e-10))
plot(pca.emf$d) #~5 components
plot(scale(pca.emf$u[,1:2]))

# com.mf = getComMembership(pca.emf$u[,1:5], k=300, method = igraph::cluster_fast_greedy)
# cluster.label = factor(com.mf)
# cell.cols.mf = MUDAN:::fac2col(cluster.label)
# plot(scale(pca.emf$u[,1:2]), pch=16, xlab = 'PC1', ylab = 'PC2', col = cell.cols.mf, main="PCA: simulated trajectories")

set.seed(1)
umap.mf = umap(pca.emf$u[,1:10])$layout
plot(umap.mf,pch=4) #,col = cell.cols.mf
...
plot(pca$u[,1:2],pch=4, col = cell.cols)

now.emb = scale(now.scores[,1:2])
proj.emb = scale(proj.scores[,1:2]) + c(0.2,-0.1)

plot(now.emb, pch = 4, xlab = "PC1", ylab = "PC2", main = "PCA on simulated data", xlim = c(-2,2.5), ylim = c(-3,2.5))
...
legend(x = -2, y = 2, c("exp","proj","exp","proj"), col = c("black", "blue","red","green"), pch = c(4,4,16,16))
...
```{r plot panc data on uma, eval=FALSE}
...
```{r extract counts, message = FALSE, results='hide', eval=FALSE}
...
```{r scvelo pca, message = FALSE, results='hide', eval=FALSE}
...
Filter genes  
```{r filter genes, message = FALSE, eval=FALSE }
...
```{r downsample, results='hide', eval=FALSE}
...
```{r normalize, results='hide', eval=FALSE}
...
```{r dimensional reduction, eval=FALSE}
...
```{r umap, fig.height=5, fig.width=7, eval=FALSE}
...
```{r velocity, message = FALSE, results = 'hide', eval=FALSE}
...
```{r graph viz from PCs, results = "hide", eval=FALSE}
...
```{r removing pre-endocrine cells, eval=FALSE}
...
```{r removing ngn3 high EP cells, eval=FALSE}
...
```{r removing ngn3 high EP and preendocrine cells, eval=FALSE}
...
```{r removing beta cells, eval=FALSE}
...
```{r removing some beta cells, eval=FALSE}
not.beta.bars = names(clusters)[which(clusters != "Beta")]
set.seed(2)
