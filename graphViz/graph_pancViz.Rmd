...
# plot(pcs[,1:2], col = cell.cols, pch = 16)
```
...
plot(proj.scores[,1:2], col = cell.cols.sub, main = 'Projected')

```

Graph visualization on subset of cells from PC coordinates  
```{r graph viz from PCs, results = "hide", fig.width=6, fig.height=5}
set.seed(1)
cell.names = colnames(curr)
cell.names.sub = cell.names[seq(1,length(cell.names),1)] #downsample cells for speed ### CHANGE HERE TO CHANGE SUBSAMPLE SIZE
...
g = graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         ...
         cell.colors = cell.cols.grph, title = "graph using PCA coordinates", 
         plot = TRUE, return_graph = TRUE)

```
```{r}
fdg3d = layout_with_fr(g$graph,dim=3) 
rglplot(g$graph, layout = layout_with_fr)
...
```{r changing distance measure, echo = TRUE, results = "hide", fig.width=5, fig.height=5}
#using k=10, similarity=cosine, threshold=0.25
set.seed(1)
graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = 50, distance_metric = "L1", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
         cell.colors = cell.cols.grph, title = "L1 Distance",
         plot = TRUE, return_graph = FALSE)
set.seed(1)
graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = 50, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
         cell.colors = cell.cols.grph, title = "L2 Distance",
         plot = TRUE, return_graph = FALSE)
```

Pearson correlation vs Cosine similarity:  
```{r changing similarity measure, results = "hide", echo = TRUE, eval = FALSE}
set.seed(1)
graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
...
set.seed(1)
graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = 10, distance_metric = "L2", similarity_metric = "pearson", similarity_threshold = -0.5, weighted = TRUE, 
...
```{r changing k, results = "hide", fig.width=4, fig.height=5}
#using distance=L2, similarity=cosine, threshold=0.25
ks = c(5,10,50,100,250)
for (k in ks){
  graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = k, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
         cell.colors = cell.cols.grph, title = paste("K = ",k),
         plot = TRUE, return_graph = FALSE)
...
for (t in ts){
  graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
         k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = t, weighted = TRUE, 
         cell.colors = cell.cols.grph, title = paste("Similarity Threshold = ",t),
         plot = TRUE, return_graph = FALSE)
...
```{r neighbors on fdg, results = 'hide', eval=FALSE}
fdg = graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
               k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
               cell.colors = cell.cols.grph, title = NA,
               plot = TRUE, return_graph = TRUE)
...
Number of out edges k:  
```{r, results = "hide", fig.width=9, fig.height=7}
#using distance=L2, similarity=cosine, threshold=0.25
deltaExp = vel$deltaE[common.genes.names,cell.names.sub]  ### common.genes.names = intersect of overdispersed genes and velocity genes (good correlation b/w u and s)
...
for (i in seq(1,length(ks))){
  ...
  set.seed(1)
  currG = graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
                   k = k, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
                   cell.colors = cell.cols.grph, title = paste("K = ",k),
...
xt = c(-0.5,-0.5)
yt = c(70,35)
hist(conScores_k[,1],col="red",breaks = b, xlim = x, ylim = y, xlab = "Cell Consistency Score",main="K = 1")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_k[,1]))),paste("Median:\n",as.character(median(conScores_k[,1])))))
hist(conScores_k[,2],col="blue",breaks = b,xlim = x, ylim = y, xlab = "Cell Consistency Score",main="K = 5")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_k[,2]))),paste("Median:\n",as.character(median(conScores_k[,2])))))
hist(conScores_k[,3],col="green",breaks = b, xlim = x, ylim = y, xlab = "Cell Consistency Score",main="K = 10")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_k[,3]))),paste("Median:\n",as.character(median(conScores_k[,3])))))
hist(conScores_k[,4],col="yellow",breaks = b,xlim = x, ylim = y, xlab = "Cell Consistency Score",main="K = 50")
text(x = xt, y = yt,c(paste("Mean:\n",as.character(mean(conScores_k[,4]))),paste("Median:\n",as.character(median(conScores_k[,4])))))
hist(conScores_k[,5],col="black",breaks = b, xlim = x, ylim = y,  xlab = "Cell Consistency Score",main="K = 100") 
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_k[,5]))),paste("Median:\n",as.character(median(conScores_k[,5])))))
hist(conScores_k[,6],col="cyan",breaks = b, xlim = x, ylim = y, xlab = "Cell Consistency Score",main="K = 250") 
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_k[,6]))),paste("Median:\n",as.character(median(conScores_k[,6])))))
...
xt = c(-0.5,-0.5)
yt = c(70,35)
hist(conScores_t[,1],col="red",breaks = b,xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = -1")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,1]))),paste("Median:\n",as.character(median(conScores_t[,1])))))
hist(conScores_t[,2],col="blue",breaks = b, xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = 0")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,2]))),paste("Median:\n",as.character(median(conScores_t[,2])))))
hist(conScores_t[,3],col="green",breaks = b, xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = 0.25")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,3]))),paste("Median:\n",as.character(median(conScores_t[,3])))))
hist(conScores_t[,4],col="yellow",breaks = b, xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = 0.5")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,4]))),paste("Median:\n",as.character(median(conScores_t[,4])))))
hist(conScores_t[,5],col="black",breaks = b, xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = 0.7")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,5]))),paste("Median:\n",as.character(median(conScores_t[,5])))))
hist(conScores_t[,6],col="cyan",breaks = b, xlim = x, ylim = y,xlab = "Cell Consistency Score",main="t = 0.9")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(conScores_t[,6]))),paste("Median:\n",as.character(median(conScores_t[,6])))))
...
#pca
all.logODS.sub = all.logODS[cell.names.sub,]  #common.genes.names subset?? 
...
#umap
pcs.sub = pca.sub$u[,1:20]
set.seed(1)
emb.umap.sub = umap(pcs.sub)$layout
...
set.seed(1)
fdgCoords = graphViz(observed = curr.scores.cellsub, projected = proj.scores.cellsub,
                     k = 10, distance_metric = "L2", similarity_metric = "cosine", similarity_threshold = 0.25, weighted = TRUE, 
                     cell.colors = cell.cols.grph, title = NA,
                     plot = FALSE, return_graph = TRUE)$fdg_coords
...
xt = c(-0.25,-0.25)
yt = c(70,35)
xl = "Cell Consistency Score"
hist(pcaCon,breaks = b, col = "red",xlim = x, ylim = y, xlab =  xl, main = "PCA Embedding Consistency")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(pcaCon))),paste("Median:\n",as.character(median(pcaCon)))))
hist(umapCon,breaks = b, col = "green",xlim = x, ylim = y,xlab =  xl, main = "UMAP Embedding Consistency")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(umapCon))),paste("Median:\n",as.character(median(umapCon)))))
hist(fdgCon, breaks = b, col = "blue",xlim = x, ylim = y,xlab =  xl, main = "FDG Embedding Consistency")
text(x = xt, y = yt, c(paste("Mean:\n",as.character(mean(fdgCon))),paste("Median:\n",as.character(median(fdgCon)))))
