...
adata.missing = py_load_object("./saved_objects/cellrank_panc_adata_missing_fev.rds")
...
cell.cols.missing  = cell.cols[names(clusters.missing)]
```
...
emb_umap = adata.missing$obsm['X_umap'] #extract umap embedding
clusters.missing = adata.missing$obs$clusters #extract clusters
rownames(emb_umap) = names(clusters.missing) = adata.missing$obs_names$values
...
legend(x=10, y=6, legend = levels(clusters.missing), col = col, pch = 16)

emb_umap = adata.missing$obsm['X_umap'] #extract umap embedding 
plot(emb_umap, col = cell.cols.missing, pch=16, xlab = "UMAP X", ylab = "UMAP Y")
legend(x=10, y=0, legend = levels(clusters.missing), col = col, pch = 16, cex = 0.7)
...
# py_save_object(adata.missing, filename = "./saved_objects/cellrank_panc_adata_missing_fev.rds")
# adata.missing2 = py_load_object("./saved_objects/cellrank_panc_adata_missing_fev.rds")

```

Extract transition matrices  
```{r transition matrices missing, fig.width=7, fig.height=6, fig.width = 4}
...
Combined: transition matrix = weighted average of distance and velocity  
(using default weight 0.8velocity + 0.2distance)
```{r comapring graph types missing, fig.width=7, fig.height=3}
par(mfrow = c(1,3))
...
Varying weight in weighted average   
```{r different weights missing, fig.width=7, fig.height=9}
ws = c(0,0.25,0.5,0.75,0.8,1)
...
plot(emb.pca.missing, col = cell.cols.missing.sub, pch = 16, xlab = "PC1", ylab = "PC2", main = "PCA")
...
# saveRDS(vel.missing,file = "./saved_objects/panc_vel_cellrank_missing_fev.rds")
...
plot(emb.pca.velo.missing, col = cell.cols.missing.sub, pch = 16, xlab = "PC1", ylab = "PC2", main = "PCA")
...
plot(curr.scores.missing[,1:2], pch = 16, col = cell.cols.missing.sub, main = 'Observed')#,xlim = c(-0.55,0.25),ylim = c(-0.35,0.45))
...
plot(proj.scores.missing[,1:2], pch = 16, col = cell.cols.missing.sub, main = 'Projected')#,xlim = c(-0.55,0.25),ylim = c(-0.35,0.45))
legend(x=-5, y=20, legend = levels(clusters), col = col, pch = 16, cex = 0.7)
...
for (w in ws){
  set.seed(1)
  vizg = graphViz(t(curr.scores.missing), t(proj.scores.missing), k=30, distance_metric = "L2", similarity_metric = "cosine", distance_weight = w, similarity_threshold = 0, weighted = TRUE, cell.colors = cell.cols.missing.sub,plot = TRUE, return_graph = TRUE,title = paste("Weight = ", w))
}
...
for (k in ks){
  ...
  vizg = graphViz(t(curr.scores.missing), t(proj.scores.missing), k=k, 
                  ...
                  cell.colors = cell.cols.missing.sub, plot = TRUE, return_graph = TRUE,title = paste("K = ", k))
}
```
...
vizg = graphViz(t(curr.scores.missing), t(proj.scores.missing), k=k,
                distance_metric = "L2", similarity_metric = "cosine", distance_weight = 1, similarity_threshold = 0, weighted = TRUE, 
                cell.colors = cell.cols.missing.sub, plot = FALSE, return_graph = TRUE,title = paste("veloViz: K = ", k))
viz.fdg = vizg$fdg_coords
plot(viz.fdg, pch = 16, col = cell.cols.missing.sub, main = paste("veloViz: K = ", k,"\ndirect comparison"), cex = 0.5)
...
vizg = graphViz(t(curr.scores.missing), t(proj.scores.missing), k=k,
                ...
                cell.colors = cell.cols.missing.sub, plot = FALSE, return_graph = TRUE,title = paste("veloViz: K = ", k))
viz.fdg = vizg$fdg_coords
plot(viz.fdg, pch = 16, col = cell.cols.missing.sub, main = paste("veloViz: K = ", k), cex = 0.5)
