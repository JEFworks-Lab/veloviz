...
Here, we are simulating branching trajectories and comparing velocity based embedding to other embeddings with all cells and with missing intermediates. 
...
# 
# 
# plot(tc,uc,col = "black",xlim = c(-0.05, 1.15), ylim = c(-1,1))
# points(tp,up, col = "red", pch = 4)
# text(tc[seq(1,700,50)],uc[seq(1,700,50)]+0.1, label = seq(1,700,50), pch = 16, col = "blue")
# text(tp[seq(1,700,50)],up[seq(1,700,50)]+0.1, label = seq(1,700,50), pch = 16, col = "green")
...
#svg("three_branch_all.svg", width = 16, height = 3.5)
par(mfrow = c(1,6))
...
#### PLOTTING ####
```{r plot no int cells ,fig.width=8, fig.height=8 }
#svg("two_branch_noInt_2.svg", width = 8, height = 8)
layout(matrix(c(1,2,2,1,2,2,3,4,5), nrow = 3, ncol = 3, byrow = TRUE))
#par(mfrow = c(1,6))
plot(tc,uc,pch = 20, col = cell.cols, xlim = c(-0.1, 1.2), ylim = c(-1,1), xlab = "", ylab = "",xaxt='n',yaxt='n', frame.plot = FALSE)
plot(g$fdg_coords,pch=16, xlab = '', ylab = '', col = cell.cols, xaxt='n',yaxt='n')
title(xlab = "Velo 1", ylab = "Velo 2", line = 1,cex.lab = 2)
