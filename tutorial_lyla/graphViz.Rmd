...
emb.test.sub.names = rownames(emb.test.ord)[seq(1,1368,by=5)]
...
#### Construct graph for cell subset 
...
projectedNeighbors = function(observed,projected,k,distance_metric="L2",similarity_metric="cosine"){
...
nns = projectedNeighbors(obs,exp,1,"L2","pearson")

text(t(obs)+c(0,-0.2), labels = paste("proj neighbor:",nns$NNs))
#text(t(obs)+c(0.2,-0.2), labels = paste("invL1:",round(min(nns$dist_comp$invDist,na.rm = TRUE),2)))

edgeListTest = cbind(seq(1:5),nns$kNNs[,1])
#edgeListTest = rbind(cbind(seq(1:5),nns$kNNs[,1]),cbind(seq(1:5),nns$kNNs[,2]))
...
k=20
pcaSubNNs = projectedNeighbors(curr.scores.cellsub,proj.scores.cellsub,k,"L2","pearson")
...
```{r k>1 gene expression coordinates}
set.seed(1)

k = 20
gexpSubNNs = projectedNeighbors(curr.sub,proj.sub,k,"L1","pearson")
