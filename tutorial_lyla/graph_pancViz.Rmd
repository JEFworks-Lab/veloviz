...
```{r plot on different embeddings}
...
```{r calculate velocity in scvelo}
...
#### Consistency score
```{r neighbors on fdg }
fdg = graphViz(curr.scores.cellsub,proj.scores.cellsub,10,"L2","cosine",0.25,cell.cols.grph,plot=FALSE,return_graph = TRUE)
g = fdg$graph
...
plot(coords)
cell = 5
points(coords[cell,1],coords[cell,2],col = "blue",pch = 4)
points(coords[neighbors[cell,],1],coords[neighbors[cell,],2], col = "red",pch = 4)
...
hist(cellConsistency,breaks = 100)
```
...
cellcon = consistency(coords,deltaExp,10,TRUE)
