...
set.seed(0)
emb.umap = uwot::umap(t(obs), n_neighbors = 10)
plot(emb.umap, col=col, pch=16)

## 2D embedding by veloviz
vig = veloviz::buildVeloviz(
  curr = t(obs),
  proj = t(exp),
  normalize.depth = FALSE, ## no dimensionality reduction
  use.ods.genes = FALSE, ## no dimensionality reduction
...
g = veloviz::plotVeloviz(vig, col=col, seed=0)
```
